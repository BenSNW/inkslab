...
from inkslab.python.dataset.handle_conll_data import save_instance, load_instance, load_data
...
def get_feature_position(config, feat):
    ...
    if re.match(RE_STACK, feat):  # STACK[i]
        ...
        if len(groups) == 3:
            ...
            stack_id = int(groups[2])
            word_id = config.get_right_child(config.get_right_child(config.get_stack(stack_id), rc_id_2 + 1),
                                             rc_id_1 + 1)
            return word_id
    else:
...
def build_examples(path=None, is_train=True):
    sents, trees, vocab_dict, pos_dict, label_dict, feature_tpl = load_data(path, is_train)
    if is_train:
        x_instance_path = os.path.join(path, "train_examples_X.pkl")
        y_instance_path = os.path.join(path, "train_examples_Y.pkl")
    else:
        x_instance_path = os.path.join(path, "dev_examples_X.pkl")
        y_instance_path = os.path.join(path, "dev_examples_Y.pkl")
    ...
    print("DEBUG: Saved Example Instance Path %s" % path)
    save_instance(x_instance_path, X)
    save_instance(y_instance_path, Y)
    return X, Y


def get_examples(instance_path, is_train=True):
    if is_train:
...
if __name__ == "__main__":
    ...
    build_examples(FLAGS.data_dir, True)
    build_examples(FLAGS.data_dir, False)
