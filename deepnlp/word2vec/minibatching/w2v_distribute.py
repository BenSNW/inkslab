...
class Word2Vec(object):
    ...
    def build_graph(self):
        ...
        self.sv = tf.train.Supervisor(is_chief=(opts.task_index == 0),
                                      logdir="/tmp/dist-w2v",
    ...
    def _train_thread(self, coord, session):
        try:
            while not coord.should_stop():
                session.run([self._train])
    ...
    def train(self, session):
        ...
        for _ in range(opts.parallel):
            worker = threading.Thread(target=self._train_thread, args=(coord,session,))
            worker.start()
        ...
        try:
            while not coord.should_stop():
                ...
                if t_now - last_eval_time > opts.eval_time_interval and opts.task_index == 0:
                    self.nearby(self._valid_words, session, num=8)
                    last_eval_time = t_now

                if t_now - last_save_time > opts.save_time_interval and opts.task_index == 0:
                    self.save_model(session, stamp)
    ...
    def pick_valid_words(self, valid_words_count=8):
        ...
        if not opts.valid_words:
            ...
            while len(valid_words) < valid_words_count:
                w_id, = np.random.choice(range(10, 20), 1)
    ...
    def nearby(self, words, session, num=20):
        ids = np.array([self._word2id.get(x, UNK_ID) for x in words])
        vals, idx = session.run(
    ...
    def save_model(self, session, stamp):
        opts = self._options
        nemb, = session.run([self._nemb])
        with open(opts.save_path + "_" + stamp, 'w') as fout:
...
def main(args):
    opts = args
    print("Workers = %s" % opts.workers)
    cluster = tf.train.ClusterSpec({"ps": opts.parameter_servers.split(","),
                                    "worker": opts.workers.split(",")})
    ...
    if opts.job_name == 'ps':
        ...
        model = Word2Vec(options=args)
        device_setter = tf.train.replica_device_setter(
            worker_device="/job:worker/task:%d" % opts.task_index, cluster=cluster)
        with tf.device(device_setter):
            model.init()
            with model.sv.prepare_or_wait_for_session(server.target, model.sess_config) as session:
                model.train(session)
                if opts.task_index == 0:
                    model.save_model(session, "finished")
        model.sv.stop()

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--workers', required=True, type=str)
    parser.add_argument('--parameter_servers', required=True, type=str)
    parser.add_argument('--job_name', required=True, type=str, default='ps')
    parser.add_argument('--task_index', required=True, type=int, default=0)

    parser.add_argument('--train_dir', required=True)
    parser.add_argument('--save_path', required=True)
    parser.add_argument('--valid_words', type=str)

    parser.add_argument('--epochs_to_train', type=int, default=100)
