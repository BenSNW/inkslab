...
class CNNModel(object):
    def __init__(self, config):
        ...
        self._loss, self._accuracy, self._predict_y = self.build_model()
    ...
    def build_model(self):
        ...
        predict_y = tf.round(tf.nn.sigmoid(scores))
        correct_prediction = tf.equal(predict_y, tf.round(self._y_in))
        accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))

        return loss, accuracy, predict_y
