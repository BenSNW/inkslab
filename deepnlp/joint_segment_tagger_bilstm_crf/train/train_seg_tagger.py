...
import os, time
from deepnlp.joint_segment_tagger_bilstm_crf.dataset import data_transform
from deepnlp.joint_segment_tagger_bilstm_crf.model import Model
...
def train(args):
    ...
    if args.ngram > 1 and not os.path.isfile(args.vocab_dir + '/' + str(args.ngram) + 'gram.txt') \
        ...
        data_transform.get_vocab_tag(args.vocab_dir, [train_file, dev_file], ngram=args.ngram)

    # 字集，标签集，n元词集
    chars, tags, ngram = data_transform.read_vocab_tag(args.vocab_dir, args.ngram)
    ...
    if args.pre_embeddings:
        ...
        if not os.path.isfile(args.vocab_dir + '/' + short_emb + '_sub.txt'):
            data_transform.get_sample_embedding(glove_file, args.vocab_dir, short_emb, chars)
            emb_dim, emb = data_transform.read_sample_embedding(args.vocab_dir, short_emb)
    ...
    train_x, train_y, train_max_slen_c, train_max_slen_w, train_max_wlen = \
        data_transform.get_input_vec(train_file, char2idx, tag2idx, args.tag_scheme)

    dev_x, dev_y, dev_max_slen_c, dev_max_slen_w, dev_max_wlen = \
        data_transform.get_input_vec(dev_file, char2idx, tag2idx, args.tag_scheme)
    ...
    b_train_x, b_train_y = data_transform.buckets(train_x, train_y, size=args.bucket_size)
    b_dev_x, b_dev_y = data_transform.buckets(dev_x, dev_y, size=args.bucket_size)
    ...
    nums_tags = data_transform.get_nums_tags(tag2idx, args.tag_scheme)
