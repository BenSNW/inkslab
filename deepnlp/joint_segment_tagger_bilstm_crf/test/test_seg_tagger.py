...
def joint_predict(args):
    ...
    chars, tags, grams = data_transform.read_vocab_tag(args.train_dir, ngram)
    char2idx, idx2char, tag2idx, idx2tag = data_transform.get_dic(chars, tags)
    new_chars, new_grams, new_gram_emb, gram2idx = None, None, None, None
    raw_x = None

    new_chars = data_transform.get_new_chars(args.predict_file, char2idx, type='raw')
    char2idx, idx2char = data_transform.update_char_dict(char2idx, new_chars)

    if not args.tag_large:
        raw_x, raw_len = data_transform.get_input_vec_raw(args.predict_file, char2idx)
        print('Numbers of sentences: %d.' % len(raw_x[0]))
        max_step = raw_len
    else:
        max_step = data_transform.get_maxstep(args.predict_file)

    print('Longest sentence is %d. ' % max_step)
    if ngram > 1:
        gram2idx = data_transform.get_ngram_dic(grams)
        if not args.tag_large:
            raw_gram = data_transform.get_gram_vec(args.predict_file, gram2idx, is_raw=True)
            raw_x += raw_gram
    if not args.tag_large:
