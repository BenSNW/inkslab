...
import os
from deepnlp.lstm_pos.dataset import rawdata, dataset
from deepnlp.lstm_pos import pos_model
from deepnlp.lstm_pos import pos_model_bilstm
...
def train_lstm(args):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        with tf.variable_scope('pos_var_scope', reuse=None, initializer=initializer):
            m = pos_model.POSTagger(is_training=True, config=args)
        with tf.variable_scope('pos_var_scope', reuse=True, initializer=initializer):
            valid_m = pos_model.POSTagger(is_training=False, config=args)
        ...
        for i in range(args.num_epochs):
            ...
            train_perplexity = pos_model.run(sess, m,
                                             train_dataset, m.train_op, pos_train_dir=args.train_dir, epoch=i)
            print("Epoch: %d Train Perplexity: %.3f" % (i + 1, train_perplexity))

            valid_perplexity = pos_model.run(sess, valid_m,
...
def train_bilstm(args):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        initializer = tf.random_normal_initializer(-args.init_scale, args.init_scale)
        with tf.variable_scope('pos_var_scope', reuse=None, initializer=initializer):
            m = pos_model_bilstm.POSTagger(is_training=True, config=args)
        with tf.variable_scope('pos_var_scope', reuse=True, initializer=initializer):
            valid_m = pos_model_bilstm.POSTagger(is_training=False, config=args)
