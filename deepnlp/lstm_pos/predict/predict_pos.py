...
from deepnlp.lstm_pos import pos_model , pos_model_bilstm
from deepnlp.lstm_pos.dataset import rawdata
...
class ModelLoader(object):
    ...
    def init_ner_model(self, session, ckpt_path):
        ...
        with tf.variable_scope("pos_var_scope"):
            if self.method == "lstm":
                model = pos_model.POSTagger(is_training=False, config=self.config)
            else:
                model = pos_model_bilstm.POSTagger(is_training=False, config=self.config)
