...
from deepnlp.lstm_ner.dataset import rawdata
...
class ModelLoader(object):
    ...
    def init_ner_model(self, session, ckpt_path):
        ...
        with tf.variable_scope("ner_var_scope"):
            if self.method == "lstm":
                model = ner_model.NERTagger(is_training=False, config=self.config)
            else:
                model = ner_model_bilstm.NERTagger(is_training=False, config=self.config)
