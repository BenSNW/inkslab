...
from deepnlp.idcnn_or_bilstm_crf_segment.stand_alone.utils import result_to_sentence
...
class Model(object):
    ...
    def predict_line(self, sess, inputs, id_to_tag):
        trans = self.trans.eval()
    ...
    def predict_batch(self, sess, data_manager, id_to_tag, id_to_char):
        results = []
        trans = self.trans.eval()
        for batch in data_manager.iter_batch():
            strings = batch[0]
            ...
            for i in range(len(strings)):
                string = strings[i][:lengths[i]]
                string = [id_to_char[c] for c in string]
                pred = [id_to_tag[int(x)] for x in batch_paths[i][:lengths[i]]]

                result = result_to_sentence(string, pred)
